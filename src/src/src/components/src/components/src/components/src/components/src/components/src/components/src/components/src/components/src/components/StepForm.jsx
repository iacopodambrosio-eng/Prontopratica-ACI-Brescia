    ...
    <div>
      ...
      <div className="card">
        ...
        <div className="form-row">
          <div className="form-group">
            ...
            <input className="form-input" type="text" placeholder="Mario" value={nome} onChange={e => onChange({ nome: e.target.value })} />
          </div>
          <div className="form-group">
            <label className="form-label">Cognome *</label>
            <input className="form-input" type="text" placeholder="Rossi" value={cognome} onChange={e => onChange({ cognome: e.target.value })} />
          </div>
        </div>
        ...
        <div className="form-row">
          <div className="form-group">
            ...
            <input className="form-input" type="tel" placeholder="030 1234567" value={telefono} onChange={e => onChange({ telefono: e.target.value })} />
          </div>
          <div className="form-group">
            <label className="form-label">Email <span className="optional">(opzionale)</span></label>
            <input className="form-input" type="email" placeholder="mario@email.it" value={email} onChange={e => onChange({ email: e.target.value })} />
          </div>
        </div>

        <div className="form-group">
          <label className="form-label">Note <span className="optional">(opzionale)</span></label>
          <textarea className="form-input form-textarea" placeholder="Informazioni aggiuntive sulla pratica..." value={note} onChange={e => onChange({ note: e.target.value })} rows={3} />
        </div>

        <p className="privacy-note">* Campi obbligatori. I dati verranno utilizzati esclusivamente per la gestione dell'appuntamento.</p>
      ...
      <div className="btn-row">
        ...
        <button className="btn-primary" disabled={!valid || loading} onClick={handleSubmit}>
          {loading ? (<><span className="btn-spinner" />Conferma in corso...</>) : ('Conferma appuntamento ✓')}
        </button>
      </div>
    </div>
  )
